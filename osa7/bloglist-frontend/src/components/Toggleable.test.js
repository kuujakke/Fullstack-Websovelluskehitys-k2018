...
it('should display blog-details when clicked', () => {
    ...
    expect(wrapper.find('.toggle-content').props().style.display).
        toBe('none')
    wrapper.find(Toggleable).find('.toggle').simulate('click')
    wrapper.update()
    expect(wrapper.find('.toggle-header').props().style.display).
        toBe('none')
    expect(wrapper.find('.toggle-content').props().style.display).toBe('')
    expect(
        wrapper.find('.toggle-content').children().at(0).props().className).
        toBe('blog-details')
